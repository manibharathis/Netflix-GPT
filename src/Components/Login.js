  ...
  return (
    <div>
      ...
      {isSignInForm? <input  className="p-4 my-4 w-full bg-gray-700" type="string"  placeholder='Name' />:""}
        <input  className="p-4 my-4 w-full bg-gray-700" ref={email} type="email"  placeholder='Email' />
        <input  className="p-4 my-4 w-full bg-gray-700" ref={password} type="password"  placeholder='Password'/>
        {formError!=null?<p className='text-red-600 font-bold text-lg py-2'>{formError}</p>:""}
        <button onClick={handleClick} className="p-4 my-6 bg-red-700 w-full rounded-lg">{isSignInForm?"SignUp":"SignIn"}</button>
